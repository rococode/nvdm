...
flags.DEFINE_integer('epochs', 1000, 'num epochs.')
...
def train(sess, model, 
  ...
  for epoch in range(training_epochs):
    ...
    if print_ppx < best_dev_ppx:
        best_dev_ppx = print_ppx
        best_dev_epoch = epoch + 1
        print('Found new best dev')
	...
	doc_count += np.sum(mask) 
      ...
      if print_ppx < best_test_ppx:
        best_test_ppx = print_ppx
        best_test_epoch = epoch + 1
        print('Found new best test')
      print('| Epoch test: {:d} |'.format(epoch+1), 
	     '| Perplexity: {:.9f}'.format(print_ppx),
	     '| Per doc ppx: {:.5f}'.format(print_ppx_perdoc),
	     '| KLD: {:.5}'.format(print_kld))   

    print("best dev perplexity:", best_dev_ppx, "epoch:", best_dev_epoch)
    print("best test perplexity:", best_test_ppx, "epoch:", best_test_epoch)
...
def main(argv=None):
    ...
    train(sess, nvdm, train_url, test_url, FLAGS.batch_size, training_epochs=FLAGS.epochs)
